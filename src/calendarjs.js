function calendarJs( id, options, searchOptions, startDateTime ) {
    ...
    function buildDateHeader() {
        ...
        if ( _options.showExtraMainDisplayToolbarButtons ) {
            buildToolbarButton( _element_HeaderDateDisplay, _options.refreshTooltipText, refreshViews );
        ...
        if ( _options.showExtraMainDisplayToolbarButtons ) {
            ...
            if ( _options.exportEventsEnabled ) {
                _element_HeaderDateDisplay_ExportEventsButton = buildToolbarButton( _element_HeaderDateDisplay, _options.exportEventsTooltipText, function() {
    ...
    function buildFullDayView() {
        ...
        if ( _options.exportEventsEnabled ) {
            _element_FullDayView_ExportEventsButton = buildToolbarButton( titleBar, _options.exportEventsTooltipText, function() {
    ...
    function buildListAllEventsView() {
        ...
        if ( _options.exportEventsEnabled ) {
            _element_ListAllEventsView_ExportEventsButton = buildToolbarButton( titleBar, _options.exportEventsTooltipText, function() {
    ...
    function buildListAllWeekEventsView() {
        ...
        if ( _options.exportEventsEnabled ) {
            _element_ListAllWeekEventsView_ExportEventsButton = buildToolbarButton( titleBar, _options.exportEventsTooltipText, function() {
    ...
    function setTranslationStringOptions() {
        if ( !isDefinedString( _options.previousMonthTooltipText ) ) {
            _options.previousMonthTooltipText = "Предыдущий месяц";
        }

        if ( !isDefinedString( _options.nextMonthTooltipText ) ) {
            _options.nextMonthTooltipText = "Следующий месяц";
        }

        if ( !isDefinedString( _options.previousDayTooltipText ) ) {
            _options.previousDayTooltipText = "Предыдущий день";
        }

        if ( !isDefinedString( _options.nextDayTooltipText ) ) {
            _options.nextDayTooltipText = "Следующий день";
        }

        if ( !isDefinedString( _options.previousWeekTooltipText ) ) {
            _options.previousWeekTooltipText = "Предыдущая неделя";
        }

        if ( !isDefinedString( _options.nextWeekTooltipText ) ) {
            _options.nextWeekTooltipText = "Следующая неделя";
        }

        if ( !isDefinedString( _options.addEventTooltipText ) ) {
            _options.addEventTooltipText = "Добавить событие";
        }

        if ( !isDefinedString( _options.closeTooltipText ) ) {
            _options.closeTooltipText = "Закрыть";
        }

        if ( !isDefinedString( _options.exportEventsTooltipText ) ) {
            _options.exportEventsTooltipText = "Экспорт событий";
        }

        if ( !isDefinedString( _options.todayTooltipText ) ) {
            _options.todayTooltipText = "Сегодня";
        }

        if ( !isDefinedString( _options.refreshTooltipText ) ) {
            _options.refreshTooltipText = "Обновить";
        }

        if ( !isDefinedString( _options.searchTooltipText ) ) {
            _options.searchTooltipText = "Поиск";
        }

        if ( !isDefinedString( _options.expandDayTooltipText ) ) {
            _options.expandDayTooltipText = "Расширить день";
        }

        if ( !isDefinedString( _options.listAllEventsTooltipText ) ) {
            _options.listAllEventsTooltipText = "Просмотреть все события";
        }

        if ( !isDefinedString( _options.listWeekEventsTooltipText ) ) {
            _options.listWeekEventsTooltipText = "Просмотр событий текущей недели";
        }
        ...
        if ( !isDefinedString( _options.fromTimeErrorMessage ) ) {
            _options.fromTimeErrorMessage = "Пожалуйста, выберите действительное время 'От'.";
        }

        if ( !isDefinedString( _options.toTimeErrorMessage ) ) {
            _options.toTimeErrorMessage = "Пожалуйста, выберите действительное время 'До'.";
        }

        if ( !isDefinedString( _options.toSmallerThanFromErrorMessage ) ) {
            _options.toSmallerThanFromErrorMessage = "Пожалуйста, выберите дату 'До', которая больше, чем дата 'От'.";
        }

        if ( !isDefinedString( _options.titleErrorMessage ) ) {
            _options.titleErrorMessage = "Пожалуйста, введите значение в поле 'Название' (без пустого места).";
        }
        ...
        if ( !isDefinedString( _options.allDayText ) ) {
            _options.allDayText = "Весь день";
        }

        if ( !isDefinedString( _options.allEventsText ) ) {
            _options.allEventsText = "Все события";
        }
        ...
        if ( !isDefinedString( _options.confirmEventRemoveTitle ) ) {
            _options.confirmEventRemoveTitle = "Подтвердите удаление события";
        }
        
        if ( !isDefinedString( _options.confirmEventRemoveMessage ) ) {
            _options.confirmEventRemoveMessage = "Удаление этого события не может быть отменено. Вы хотите продолжить?";
        }
        ...
        if ( !isDefinedString( _options.expandMonthTooltipText ) ) {
            _options.expandMonthTooltipText = "Расширить месяц";
        }
        
        if ( !isDefinedString( _options.repeatEndsText ) ) {
            _options.repeatEndsText = "Повторение заканчивается:";
        }
        
        if ( !isDefinedString( _options.noEventsAvailableText ) ) {
            _options.noEventsAvailableText = "Доступных событий нет.";
        }
        
        if ( !isDefinedString( _options.viewWeekEventsText ) ) {
            _options.viewWeekEventsText = "Просмотр событий недели";
        }
        
        if ( !isDefinedString( _options.noEventsAvailableFullText ) ) {
            _options.noEventsAvailableFullText = "Нет доступных для просмотра событий.";
        }
        
        if ( !isDefinedString( _options.clickText ) ) {
            _options.clickText = "нажатие";
        }
        
        if ( !isDefinedString( _options.hereText ) ) {
            _options.hereText = "здесь";
        }
        
        if ( !isDefinedString( _options.toAddANewEventText ) ) {
            _options.toAddANewEventText = "чтобы добавить новое событие.";
        }
        
        if ( !isDefinedString( _options.weekText ) ) {
            _options.weekText = "Неделя";
        }
        
        if ( !isDefinedString( _options.groupText ) ) {
            _options.groupText = "Группа:";
        }
        
        if ( !isDefinedString( _options.configurationTooltipText ) ) {
            _options.configurationTooltipText = "Конфигурация";
        }
        
        if ( !isDefinedString( _options.configurationTitleText ) ) {
            _options.configurationTitleText = "Конфигурация";
        }
        
        if ( !isDefinedString( _options.groupsTabText ) ) {
            _options.groupsTabText = "Группа";
        }
        ...
        if ( !isDefinedString( _options.eventNotificationBody ) ) {
            _options.eventNotificationBody = "Событие '{0}' началось.";
        }

        if ( !isDefinedString( _options.optionsText ) ) {
            _options.optionsText = "Опции:";
        }

        if ( !isDefinedString( _options.startsWithText ) ) {
            _options.startsWithText = "Начинается с";
        }

        if ( !isDefinedString( _options.endsWithText ) ) {
            _options.endsWithText = "Заканчивается";
        }

        if ( !isDefinedString( _options.containsText ) ) {
            _options.containsText = "Содержит";
        }

        if ( !isDefinedString( _options.displayTabText ) ) {
            _options.displayTabText = "Дисплей";
        }
        
        if ( !isDefinedString( _options.enableAutoRefreshForEventsText ) ) {
            _options.enableAutoRefreshForEventsText = "Включить автоматическое обновление для событий";
        }

        if ( !isDefinedString( _options.enableBrowserNotificationsText ) ) {
            _options.enableBrowserNotificationsText = "Включить уведомления браузера";
        }

        if ( !isDefinedString( _options.enableTooltipsText ) ) {
            _options.enableTooltipsText = "Включить всплывающие подсказки";
        }

        if ( !isDefinedString( _options.dayText ) ) {
            _options.dayText = "день";
        }
        ...
        if ( !isDefinedString( _options.enableDragAndDropForEventText ) ) {
            _options.enableDragAndDropForEventText = "Включить перетаскивание для событий";
        }

        if ( !isDefinedString( _options.organizerTabText ) ) {
            _options.organizerTabText = "Организатор";
        }

        if ( !isDefinedString( _options.removeEventsTooltipText ) ) {
            _options.removeEventsTooltipText = "Удаление событий";
        }

        if ( !isDefinedString( _options.confirmEventsRemoveTitle ) ) {
            _options.confirmEventsRemoveTitle = "Подтвердите удаление событий";
        }
        
        if ( !isDefinedString( _options.confirmEventsRemoveMessage ) ) {
            _options.confirmEventsRemoveMessage = "Удаление этих неповторяющихся событий не может быть отменено. Вы хотите продолжить?";
        }

        if ( !isDefinedString( _options.eventText ) ) {
            _options.eventText = "Событие";
        }

        if ( !isDefinedString( _options.optionalText ) ) {
            _options.optionalText = "Расширенный";
        }

        if ( !isDefinedString( _options.urlText ) ) {
            _options.urlText = "Ссылка:";
        }

        if ( !isDefinedString( _options.openUrlText ) ) {
            _options.openUrlText = "Открыть Url";
        }

        if ( !isDefinedString( _options.visibleDaysTabText ) ) {
            _options.visibleDaysTabText = "Видимые дни";
        }

        if ( !isDefinedString( _options.enableDayNameHeadersInMainDisplayText ) ) {
            _options.enableDayNameHeadersInMainDisplayText = "Включить заголовки названий дней на главном дисплее";
        }

        if ( !isDefinedString( _options.thisWeekTooltipText ) ) {
            _options.thisWeekTooltipText = "на этой неделе";
        }

        if ( !isDefinedString( _options.dailyText ) ) {
            _options.dailyText = "Ежеднево";
        }

        if ( !isDefinedString( _options.weeklyText ) ) {
            _options.weeklyText = "Еженедельно";
        }

        if ( !isDefinedString( _options.monthlyText ) ) {
            _options.monthlyText = "Ежемесячно";
        }

        if ( !isDefinedString( _options.yearlyText ) ) {
            _options.yearlyText ="Каждый год";
        }

        if ( !isDefinedString( _options.repeatsByCustomSettingsText ) ) {
            _options.repeatsByCustomSettingsText = "По пользовательским настройкам";
        }

        if ( !isDefinedString( _options.lastUpdatedText ) ) {
            _options.lastUpdatedText = "Последнее Обновление:";
        }

        if ( !isDefinedString( _options.advancedText ) ) {
            _options.advancedText = "Расширенный";
        }

        if ( !isDefinedString( _options.copyText ) ) {
            _options.copyText = "Копировать";
        }

        if ( !isDefinedString( _options.pasteText ) ) {
            _options.pasteText = "Вставить";
        }

        if ( !isDefinedString( _options.duplicateText ) ) {
            _options.duplicateText = "Дубликат";
        }

        if ( !isDefinedString( _options.showAlertsText ) ) {
            _options.showAlertsText = "Показывать оповещения";
        }

        if ( !isDefinedString( _options.selectDatePlaceholderText ) ) {
            _options.selectDatePlaceholderText = "Выберите дату...";
        }

        if ( !isDefinedString( _options.hideDayText ) ) {
            _options.hideDayText = "Скрыть день";
        }

        if ( !isDefinedString( _options.notSearchText ) ) {
            _options.notSearchText = "Без регистра";
        }

        if ( !isDefinedString( _options.showEmptyDaysInWeekViewText ) ) {
            _options.showEmptyDaysInWeekViewText = "В разработке...";
        }

        if ( !isDefinedString( _options.showHolidaysInTheDisplaysText ) ) {
            _options.showHolidaysInTheDisplaysText = "В разработке...";
        }
    }
...
const CALENDAR_ID = 'a794eupfc4m69n86pog45li9u8@group.calendar.google.com';
const KEYFILE = 'pivotal-data-332716-ced36cd6a129.json'; // path to JSON with private key been downloaded from Google
...
(async function run() {
    ...
    async function createEvent(auth) {
        const event = {
            ...
            'start': {
                'dateTime': '2021-11-21T1:20:00+02:00',
                'timeZone': 'Europe/Riga',
            },
            'end': {
                'dateTime': '2021-11-21T1:30:00+02:00',
                'timeZone': 'Europe/Riga',
            }
        };
